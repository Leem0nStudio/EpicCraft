...
describe('Hourglass of Suspension protective modes', () => {
  ...
  it('rejects ungrouped players, neutral NPCs, pets, and dead group members', () => {
    ...
    const host = sim as unknown as { nextId: number; addEntity(e: Entity): void; rng: { range: (min: number, max: number) => number } };
    const neutral = createNpc(host.nextId++, NPCS.trader_wilkes, sim.groundPos(FLAT_X + 11, 0), host.rng);
