...
import { tickCombatCamera } from './game/camera_combat';
...
async function startGame(
    ...
    canvas,
    {
      ...
      onUiKey: (key) => {
        ...
        switch (key) {
          ...
          case 'cameraRecenter': {
            // Recenter camera behind the player (Home key, desktop only).
            // Preserves current pitch; only resets yaw to player's facing.
            input.recenterCameraBehind(world.player.facing);
            break;
          }
          case 'chat':
            openChat();
  ...
  // Share the camera state between Input and Renderer (single source of truth).
  renderer.cameraState = input.camera;
  ...
  // Camera follow state: in RO/L2-style free orbit, the camera doesn't auto-follow.
  // We still track lastInterpFacing for click-to-move smooth handoff.
  ...
  function updateCamera(frameDt: number, interpFacing: number): void {
    ...
    const next = updateFollowCameraYaw({
      ...
      orbiting: (input.leftDown || input.rightDown) && input.isCameraDragActive(),
    });
  ...
  // the move flags plus an optional forced facing (the bearing toward a click-to-move
  // destination). Any manual movement, an open modal, death, or the option being
  // switched off cancels click-to-move.
  ...
  function resolveMove(
    _mouselook: boolean, // Kept for API compatibility; always false in RO/L2-style
    ...
    attackMoveTick();
    const mi = input.readMoveInput();
    // RO/L2-style: camera never drives facing. Facing is null unless click-to-move.
    let facing: number | null = null;
    if (input.clickMoveTarget) {
      const action = resolveClickMoveAction(mi, {
        mouselook: false, // RO/L2-style: camera never drives facing
  ...
  function cameraMoveActive(): boolean {
    // RO/L2-style: Mouse Camera mode removed. Always returns false.
    return false;
  }
  ...
  function frame(now: number): void {
    ...
    perf.trace('input.cameraSmoothing', () => input.updateCameraSmoothing(frameDt), {
      frameDtMs: frameDt * 1000,
    });
    // Tick combat camera (subtle dynamic zoom/pitch when in combat).
    input.combatCamera = tickCombatCamera(input.combatCamera, !!world.player.inCombat, frameDt);
    // Sync combat camera offsets to shared CameraState for the renderer.
    input.camera.combatZoom = input.combatCamera.zoomFactor;
    input.camera.combatPitch = input.combatCamera.pitchBonus;
    ...
    // RO/L2-style: no camera-driven facing. The camera never owns the player's heading.
    // Facing comes from: controller override, keyboard turns, or the sim's own facing.
    ...
    const movementFacing = !movementFrozen() ? controllerFacing : null;
    ...
    if (offlineSim) {
      ...
      while (acc >= DT) {
        const { mi, facing } = resolveMove(
          false, // RO/L2-style: no mouselook-for-facing
      ...
      introCameraTick(now);
      // Camera state is shared (CameraState); no manual sync needed.
...
async function startOffline(
  ...
  sim.setPlayerGender(sim.playerId, offlineGender);
  ...
  startGame(sim, sim, null, `offline:${playerClass}:${name}`, true).catch((err) => {
    console.error('Offline startGame failed:', err);
    fatalOverlay(t('loading.rendererFailed', { error: technicalErrorMessage(err) }));
  });
}
...
let offlineGender: Gender = 'female'; // chosen gender for the offline quick-start character
...
function wireStartScreens(): void {
  ...
  const handleOfflineStart = (cls: PlayerClass) => {
    ...
    startOffline(cls, name, selectedSkin('#offline-skin-row', offlineSkin), undefined, undefined).catch((err) => {
      console.error('Offline startOffline failed:', err);
      hideLoadingScreen();
      offlineError.textContent = t('loading.rendererFailed', { error: technicalErrorMessage(err) });
    });
  };
  ...
  const handleOfflineSelect = () => {
    ...
    // Select novice by default and render details
    const noviceCard = document.querySelector(
      '#offline-select .mini-class[data-class="novice"]',
    ) as HTMLElement | null;
    if (noviceCard) {
      ...
      noviceCard.classList.add('sel');
      noviceCard.setAttribute('aria-pressed', 'true');
      renderClassDetails('offline-class-details', 'novice');
      btnStartOffline.removeAttribute('disabled');
      refreshOfflineSkins('novice');
    }
  };
  ...
  // offline gender buttons
  document.querySelectorAll('#offline-select .gender-btn').forEach((el) => {
    el.addEventListener('click', () => {
      document.querySelectorAll('#offline-select .gender-btn').forEach((x) => {
        x.classList.remove('sel');
      });
      el.classList.add('sel');
      offlineGender = ((el as HTMLElement).dataset.gender as Gender) || 'female';
    });
  });
  ...
  // Gender selection in character creation
  document.querySelectorAll('#charcreate-panel .gender-btn').forEach((el) => {
    el.addEventListener('click', () => {
      document.querySelectorAll('#charcreate-panel .gender-btn').forEach((x) => {
        x.classList.remove('sel');
      });
      el.classList.add('sel');
    });
  ...
  $('#btn-create-char').addEventListener('click', async () => {
    ...
    // Read selected gender (default to female)
    const genderEl = document.querySelector('#charcreate-panel .gender-btn.sel') as HTMLElement | null;
    const gender = (genderEl?.dataset.gender as Gender) || 'female';
