...
import { updateNpcCombat } from './npc_combat';
...
export interface PlayerMeta {
  ...
  /** Character gender for billboard sprite selection. */
  gender: Gender;
...
export class Sim {
  ...
  constructor(cfg: SimConfig) {
    ...
    for (const npcDef of Object.values(worldContent.npcs)) {
      ...
      const npc = createNpc(this.nextId++, npcDef, this.groundPos(safe.x, safe.z), this.rng);
      this.addEntity(npc);
    ...
    // the Honor Quartermaster cannot perturb existing entity ids or replay RNG.
    {
      ...
      if (furyDef && !this.entities.has(FURY_ENTITY_ID)) {
        ...
        const fury = createNpc(FURY_ENTITY_ID, furyDef, this.groundPos(safe.x, safe.z), this.rng);
        this.addEntity(fury);
      }
    }
  ...
  addPlayer(
    ...
    const meta: PlayerMeta = {
      ...
      gender: savedState?.gender ?? 'female',
    ...
    player.gender = meta.gender; // mirror onto the entity so the renderer + wire can read it
  ...
  spawnDevVendor(pid?: number): number {
    ...
    const npc = createNpc(this.nextId++, PTR_DEV_VENDOR_DEF, pos, this.rng);
    this.addEntity(npc);
    return npc.id;
  }
  ...
  /** Set a player's gender (meta + entity). Used by creation and the wire decode. */
  setPlayerGender(pid: number, gender: Gender): boolean {
    const meta = this.players.get(pid);
    const e = this.entities.get(pid);
    if (!meta || !e) return false;
    meta.gender = gender;
  ...
  /** Change a player's class (job change system). Only works for novice
   *  characters at level 5+. Resets talents and refreshes abilities. */
  changePlayerClass(pid: number, newClass: PlayerClass): boolean {
    const meta = this.players.get(pid);
    const e = this.entities.get(pid);
    if (!meta || !e || e.kind !== 'player') return false;
    if (meta.cls !== 'novice' || e.level < 5) return false;
    const VALID_CLASSES: PlayerClass[] = [
      'warrior', 'paladin', 'hunter', 'rogue', 'priest', 'shaman', 'mage', 'warlock', 'druid',
    ];
    if (!VALID_CLASSES.includes(newClass)) return false;
    // Change class
    meta.cls = newClass;
    e.templateId = newClass;
    // Reset talents
    meta.talents = { spec: null, rows: {} };
    meta.talentMods = computeTalentModifiers(newClass, meta.talents, e.level);
    // Recalculate stats with new class
    recalcPlayerStats(e, newClass, meta.equipment, this.ctx.playerMods(meta), meta.equipmentInstance);
    // Refresh abilities
    this.ctx.refreshKnownAbilities(meta, true);
    return true;
  }
  ...
  private buildSimContext(): SimContext {
    ...
    const host: SimContextHost = {
      ...
      // Job change system: changePlayerClass backs the /jobchange command in social/chat.ts.
      changePlayerClass: sim.changePlayerClass.bind(sim),
  ...
  tick(): SimEvent[] {
    ...
    for (const e of this.entities.values()) {
      if (e.kind === 'mob') {
        ...
        // NPC combat: fight back if attacked
        if (e.weapon && !e.dead) {
          updateNpcCombat(this.ctx, e);
        }
        // NPC idle wander: walk a short distance from spawn, then pause
        // Only wander if not in combat
        if (!e.dead && !e.wanderTarget && e.aggroTargetId === null) {
          e.wanderTimer -= DT;
          if (e.wanderTimer <= 0) {
            // Pick a random point within 2-4 yards of spawn (shorter than mob wander)
            const ang = this.rng.range(0, Math.PI * 2);
            const r = this.rng.range(2, 4);
            e.wanderTarget = this.groundPos(
              e.spawnPos.x + Math.sin(ang) * r,
              e.spawnPos.z + Math.cos(ang) * r,
            );
            e.wanderTimer = 30; // 30 seconds to reach target
          }
        }
        if (e.wanderTarget && e.aggroTargetId === null) {
          // NPCs wander at 25% speed (slower than mobs at 35%)
          const arrived = this.moveToward(e, e.wanderTarget, e.moveSpeed * 0.25);
          if (arrived) {
            e.wanderTarget = null;
            // Pause 5-15 seconds between wander trips (longer than mobs)
            e.wanderTimer = this.rng.range(5, 15);
          }
        }
  ...
  isHostileTo(attacker: Entity, target: Entity): boolean {
    ...
    if (target.kind === 'npc') {
      // Hostile NPCs (e.g., guards that attack criminals) are hostile to players
      return target.hostile;
    }
    if (target.kind === 'player') {
