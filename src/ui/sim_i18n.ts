...
const baseEnTable = {
  ...
  // Job change system (src/sim/social/chat.ts).
  'error.notNovice': 'Only novice characters can change class.',
  'error.jobChangeLevel': 'You must be at least level 5 to change class.',
  'error.invalidClass': 'Invalid class. Valid classes: {classes}.',
  'log.jobChange': 'You are now a {class}!',
...
const RULES: Rule[] = [
  ...
  // Job change system (src/sim/social/chat.ts).
  {
    re: /^Invalid class\. Valid classes: (.+)\.$/,
    build: (m) => tSim('error.invalidClass', { classes: m[1] }),
  },
  {
    re: /^You are now a (.+)!$/,
    build: (m) => tSim('log.jobChange', { class: m[1] }),
  },
];
