...
export function grantXp(
  ...
  while (p.level < MAX_LEVEL && meta.xp >= xpForLevel(p.level)) {
    ...
    // Novice class job change: when a novice reaches level 5, notify the client
    // so it can show the job change UI.
    if (meta.cls === 'novice' && p.level === 5) {
      ctx.emit({ type: 'jobChangeReady', pid: p.id });
    }
