...
const BLOOM_STRENGTH = 0.22; // reduced for RO/L2 aesthetic — subtle glow, less bloom
...
const GradeShader = {
  ...
  fragmentShader: /* glsl */ `
    ...
    uniform float uTime;
    varying vec2 vUv;
    const vec3 LIFT = vec3(0.015, 0.012, 0.010);   // warmer shadows (less blue)
    const vec3 GAIN = vec3(1.06, 1.03, 0.97);      // warm highlights (RO/L2 aesthetic)
    const vec3 GAMMA = vec3(0.96);
    void main() {
      vec3 c = texture2D(tDiffuse, vUv).rgb;
      c = pow(max(vec3(0.0), c * GAIN + LIFT), GAMMA);
      float l = dot(c, vec3(0.2126, 0.7152, 0.0722));
      c = mix(vec3(l), c, 1.08);                                  // slightly reduced saturation
      vec2 d = vUv - 0.5;
      c *= 1.0 - 0.14 * smoothstep(0.60, 0.95, dot(d, d) * 2.2);  // lighter vignette
      c += (fract(sin(dot(vUv * 731.7 + uTime, vec2(12.9898, 78.233))) * 43758.5) - 0.5) * 0.008; // softer grain
...
export function buildComposer(
  ...
  // Always enable MSAA when WebGL2 is available — even with N8AO active, the
  // composer's MSAA target provides geometry edge smoothing that bloom/grade
  // alone cannot match. The N8AO renders into its own non-MSAA beauty target,
  // but the final composite still benefits from MSAA on the output.
  ...
  if (GFX.ao) {
    ...
    if (GFX.tier === 'ultra') {
      ...
      // high tier: Low quality mode but full resolution — halfRes caused
      // significant softening on sprite-heavy scenes. Full-res Low is still
      // ~1.5ms on real GPUs and eliminates the bilinear upsample blur.
