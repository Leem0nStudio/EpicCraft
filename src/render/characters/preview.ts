...
import { SpritePreview } from './sprite_preview';
...
export class CharacterPreview {
  ...
  // Sprite preview for non-mech classes
  private spritePreview: SpritePreview | null = null;
  private spriteContainer: HTMLElement | null = null;
  ...
  /** Ensure the sprite preview container and instance exist. */
  private ensureSpritePreview(): void {
    if (this.spritePreview) return;
    // Create a dedicated container for the sprite canvas
    ...
    this.spriteContainer.style.left = '0';
    this.container.style.position = 'relative';
    this.container.appendChild(this.spriteContainer);
    this.spritePreview = new SpritePreview(this.spriteContainer);
  }

  /** Switch between sprite and 3D GLB preview mode. */
  private setPreviewMode(sprite: boolean): void {
    if (this.useSprite === sprite) return;
    this.useSprite = sprite;

    if (sprite) {
      this.ensureSpritePreview();
      // Hide 3D canvas, show sprite container
      this.canvas.style.display = 'none';
      this.spriteContainer!.style.display = '';
      this.spritePreview!.syncSize();
    } else {
      // Show 3D canvas, hide sprite container
  ...
  setClass(cls: PlayerClass, weaponItemId?: string | null, offhandItemId?: string | null): void {
    ...
    // Use sprite preview for all player classes
    this.setPreviewMode(true);
    this.spritePreview?.setClass(cls);
    // Clean up any existing 3D visual
  ...
  setAppearance(a: PreviewAppearance): void {
    ...
    // Mech skin catalog uses 3D GLB
    if (a.skinCatalog === 'mech') {
    ...
    // Non-mech uses sprite preview
    this.setPreviewMode(true);
    this.spritePreview?.setAppearance(a);
  }
  ...
  setSkin(skinIndex: number): void {
    ...
    if (this.useSprite) {
      this.spritePreview?.setSkin(skinIndex);
    } else {
  ...
  setContainer(container: HTMLElement): void {
    ...
    // Also move sprite container if it exists
    if (this.spriteContainer) {
      this.container.appendChild(this.spriteContainer);
    }
  ...
  captureCloseup(
    ...
    // Sprite mode: delegate to sprite preview
    if (this.useSprite && this.spritePreview) {
      return this.spritePreview.captureCloseup(opts);
    }
