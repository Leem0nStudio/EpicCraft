...
describe('mapJoystickVector', () => {
  ...
  it('maps diagonal movement directions', () => {
    expect(mapJoystickVector(0.7, -0.7)).toMatchObject({
      forward: true,
      back: false,
      strafeLeft: false,
      strafeRight: true,
      moveSpeedMult: expect.closeTo(0.987, 2),
    });
    expect(mapJoystickVector(-0.7, 0.7)).toMatchObject({
      forward: false,
      back: true,
...
describe('MobileControls pointer lifecycle', () => {
  ...
  it('clears movement when the active pointer ends outside the joystick element', () => {
    ...
    expect(lastMove).toEqual({ forward: false, back: false, strafeLeft: false, strafeRight: true, moveSpeedMult: 1 });
