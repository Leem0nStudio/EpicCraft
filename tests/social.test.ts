...
describe('ten classes', () => {
  it('every class spawns with a working kit and stats', () => {
    for (const cls of ALL_CLASSES) {
      ...
      // the 10-20 band still has things to learn. Exceptions: the mage baseline kit
      ...
      // review in the row-quality pass. Novice has only 3 abilities (all learned at 1).
      if (cls !== 'mage' && cls !== 'novice')
...
describe('dungeon difficulty slash command', () => {
  ...
  describe('/jobchange', () => {
    function makeNovice(name = 'Jobber') {
      const sim = makeWorld();
      const p = sim.addPlayer('novice', name);
      sim.setPlayerLevel(5, p);
      sim.drainEvents();
      return { sim, p };
    }

    function eventTexts(sim: Sim, pid: number, type: 'error' | 'log' = 'error') {
      return (sim.drainEvents() as any[])
        .filter((e) => e.type === type && e.pid === pid)
        .map((e) => e.text);
    }

    it('changes a novice to a valid target class', () => {
      const { sim, p } = makeNovice();
      sim.chat('/jobchange warrior', p);
      const texts = eventTexts(sim, p, 'log');
      expect(texts).toContain('You are now a Warrior!');
      // Meta should reflect the new class.
      const meta = (sim as any).players.get(p);
      expect(meta.cls).toBe('warrior');
    });

    it('rejects an invalid target class', () => {
      const { sim, p } = makeNovice();
      sim.chat('/jobchange invalidclass', p);
      const texts = eventTexts(sim, p);
      expect(texts.some((t: string) => t.includes('Invalid class'))).toBe(true);
    });

    it('rejects when the player is not novice', () => {
      const sim = makeWorld();
      const p = sim.addPlayer('warrior', 'AlreadyWarrior');
      sim.setPlayerLevel(5, p);
      sim.drainEvents();
      sim.chat('/jobchange mage', p);
      const texts = eventTexts(sim, p);
      expect(texts).toContain('Only novice characters can change class.');
    });

    it('rejects when the player is below level 5', () => {
      const sim = makeWorld();
      const p = sim.addPlayer('novice', 'Lowbie');
      sim.setPlayerLevel(3, p);
      sim.drainEvents();
      sim.chat('/jobchange hunter', p);
      const texts = eventTexts(sim, p);
      expect(texts).toContain('You must be at least level 5 to change class.');
    });

    it('allows all 9 main classes as valid targets', () => {
      const VALID = [
        'warrior', 'paladin', 'hunter', 'rogue', 'priest',
        'shaman', 'mage', 'warlock', 'druid',
      ];
      for (const cls of VALID) {
        const { sim, p } = makeNovice(`${cls}Test`);
        sim.chat(`/jobchange ${cls}`, p);
        const texts = eventTexts(sim, p, 'log');
        expect(texts.some((t: string) => t.includes('You are now a'))).toBe(true);
        const meta = (sim as any).players.get(p);
        expect(meta.cls).toBe(cls);
      }
    });

    it('rejects novice as a target class', () => {
      const { sim, p } = makeNovice();
      sim.chat('/jobchange novice', p);
      const texts = eventTexts(sim, p);
      expect(texts.some((t: string) => t.includes('Invalid class'))).toBe(true);
    });
  });
