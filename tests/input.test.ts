...
describe('Input camera zoom', () => {
  it('zooms the camera with the mouse wheel on desktop', () => {
    ...
    expect(input.camDist).toBeCloseTo(15.4);
  });
...
describe('Input touch invert-look', () => {
  it('reverses the touch joystick pitch when inverted, leaving yaw alone', () => {
    ...
    input.updateCameraSmoothing(1); // large dt so lerp converges fully
    ...
    (input as any).camTargetPitch = startPitch;
    (input as any).camTargetYaw = startYaw;
    input.updateTouchLook(1 / 60);
    input.updateCameraSmoothing(1);
  ...
  it('also inverts the swipe-look delta path', () => {
    ...
    // Use a moderate dy that stays within the new pitch limits [0.26, 1.05]
    // even when inverted: 0.32 - 5*0.0099 ≈ 0.27 > MIN_PITCH.
    const dy = 5;
    input.applyTouchLookDelta(0, dy);
    input.updateCameraSmoothing(1); // large dt so lerp converges fully
    const normal = input.camPitch - base;

    input.setTouchInvertLook(true);
    input.camPitch = base;
    (input as any).camTargetPitch = base;
    input.applyTouchLookDelta(0, dy);
    input.updateCameraSmoothing(1);
