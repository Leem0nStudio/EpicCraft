...
describe('render budget governor', () => {
  ...
  it('keeps the sun shadow pass while the cheap scalers give in, then drops it last', () => {
    const governor = new RenderBudgetGovernor({
      tier: 'low',
      budget: GFX_BUDGETS.low,
      enabled: true,
    ...
    // Sustained draw + frame pressure (1.18x frame, draw-dominant, grass over
    // budget) floors foliage -> grass -> lighting -> vfx in order, but stays under
    // the severe 1.25x trigger. The shadow pass is the binary knob that only gives
    // in after every cheap scaler is floored.
    const push = () =>
      sample({
        frameMs: 26,
        totalMs: 26,
    ...
    let state = governor.state();
    for (let i = 0; i < 12; i++) state = governor.update(push());
    expect(state.levels.lighting).toBeLessThan(1);
    expect(state.levels.shadow).toBe(1);

    for (let i = 0; i < 18; i++) state = governor.update(push());
    expect(state.levels.vfx).toBe(0.84); // floored
    expect(state.levels.shadow).toBe(0);
    expect(state.levels.resolution).toBeLessThan(1);
  });

  it('recovers the shadow pass first, before any other scaler', () => {
    const governor = new RenderBudgetGovernor({
      tier: 'low',
      budget: GFX_BUDGETS.low,
      enabled: true,
    ...
    // A stable, fast frame cadence recovers shadow to 1 in a single step before
    // grass even starts climbing: the biggest visual win back, cheapest first.
    for (let i = 0; i < 420; i++) {
      state = governor.update(
        sample({
          dt: 1 / 30,
