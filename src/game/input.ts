// RO/L2-style free orbit camera: left-drag orbits, scroll zooms, WASD moves
// independently. No auto-follow, no mouselook-for-facing, no Mouse Camera mode.
...
import { isPhoneTouchDevice } from './mobile_controls';
import {
...
// Camera follow speed (smooth exponential-decay lerp).
// Lower = smoother but slower convergence; higher = snappier but less fluid.
const FOLLOW_FACTOR = 0.035;
...
export interface TouchMoveInput {
  ...
  strafeLeft: boolean;
  strafeRight: boolean;
  /** 0–1 analog speed multiplier from stick deflection. */
...
export class Input {
  ...
  leftDown = false;
  rightDown = false;
  /** Shared camera state (single source of truth). */
  camera: CameraState = createInitialCameraState();
  /** Combat camera state (dynamic zoom/pitch in combat). */
  combatCamera: CombatCameraState = createCombatCameraState();
  /** Convenience getters for backward compatibility (read from shared state). */
  get camYaw(): number { return this.camera.camYaw; }
  set camYaw(v: number) { this.camera.camYaw = v; }
  get camPitch(): number { return this.camera.camPitch; }
  set camPitch(v: number) { this.camera.camPitch = v; }
  get camDist(): number { return this.camera.camDist; }
  set camDist(v: number) { this.camera.camDist = v; }
  get camTargetDist(): number { return this.camera.camTargetDist; }
  set camTargetDist(v: number) { this.camera.camTargetDist = v; }
  autorun = false;
  ...
  // Private target fields for touch/gamepad smoothing (not shared with Renderer).
  private camTargetYaw = Math.PI;
  private camTargetPitch = 0.52;
  ...
  private touchLookWasActive = false;
  ...
  /** Move the camera in/out with smooth zoom (exponential lerp toward target). */
  zoomBy(delta: number): void {
    let isMobile = false;
    try {
      isMobile = isPhoneTouchDevice();
    } catch {
      // Test environment: no matchMedia, default to desktop range.
    }
    const min = isMobile ? ZOOM_MOBILE_MIN : ZOOM_DESKTOP_MIN;
    const max = isMobile ? ZOOM_MOBILE_MAX : ZOOM_DESKTOP_MAX;
    // Update target distance and snap camDist immediately for responsive feel.
    // updateCameraSmoothing can still interpolate if multiple zooms arrive
    // between frames, but single-event zooms feel instant.
    this.camTargetDist = Math.min(max, Math.max(min, this.camTargetDist + delta));
  ...
  isMouseCameraMode(): boolean {
    // RO/L2-style: Mouse Camera mode is removed. Always returns false.
    return false;
  }
  ...
  setMouseCameraEnabled(_on: boolean): void {
    // RO/L2-style: Mouse Camera mode removed. This is now a no-op.
    // The setting UI still calls this, so we keep the method.
  }
  ...
  applyTouchLookDelta(dx: number, dy: number): void {
    ...
    this.camTargetYaw -= dx * dragSens;
    if (dy !== 0) {
      this.camTargetPitch = Math.min(
        MAX_PITCH,
        Math.max(MIN_PITCH, this.camTargetPitch + this.touchPitchSign * dy * dragSens),
      );
    }
  ...
  applyGamepadLook(yawDelta: number, pitchDelta: number): void {
    ...
    this.camTargetYaw += yawDelta;
    this.camTargetPitch = Math.min(MAX_PITCH, Math.max(MIN_PITCH, this.camTargetPitch + pitchDelta));
    this.noteIntent('look');
  }

  updateTouchLook(dt: number): void {
    if (!this.touchLookActive) {
      this.touchLookWasActive = false;
      return;
    }
    // Snap on the first frame of a new look gesture so we don't lerp from stale data.
    if (!this.touchLookWasActive) {
      this.camTargetYaw = this.camYaw;
      this.camTargetPitch = this.camPitch;
      this.touchLookWasActive = true;
    }
    this.camTargetYaw -= this.touchLookVector.x * TOUCH_LOOK_YAW_RATE * this.touchLookSpeed * dt;
    this.camTargetPitch = Math.min(
      MAX_PITCH,
      Math.max(
        MIN_PITCH,
  ...
  recenterCameraBehind(facing: number): void {
    if (Number.isFinite(facing)) {
      // Smooth recenter: set both target and current for immediate + smoothed effect.
      this.camTargetYaw = facing;
      this.camYaw = facing;
    }
    // Preserve current pitch (don't reset to default).
  }

  /** Smoothly lerp camYaw/camPitch toward touch/gamepad targets, and camDist toward zoom target. */
  updateCameraSmoothing(dt: number): void {
    const factor = 1 - Math.pow(FOLLOW_FACTOR, dt);
    // Touch/gamepad write to private targets; smooth toward them.
    this.camYaw += (this.camTargetYaw - this.camYaw) * factor;
    this.camPitch += (this.camTargetPitch - this.camPitch) * factor;
    // Smooth zoom (exponential lerp toward target distance).
    const zoomDiff = this.camTargetDist - this.camDist;
    if (Math.abs(zoomDiff) < 0.01) {
      this.camDist = this.camTargetDist;
    } else {
      this.camDist += zoomDiff * Math.min(1, ZOOM_SMOOTHING * dt);
    }
  }

  isMouselookActive(): boolean {
    // RO/L2-style: no mouselook-for-facing on desktop. Touch look on mobile
    // still counts for the strafe-on-camera-drag behavior.
  ...
  private dispatchEdge(action: string): void {
    ...
    switch (action) {
      ...
      case 'sheathe':
        ...
        return;
      case 'cameraRecenter':
        // Recenter camera behind the player (preserves pitch). Uses the player's
        // current facing from the last known position; main.ts wires this.
        this.cb.onUiKey('cameraRecenter');
        return;
    }
  }
  ...
  private onMouseMove(e: MouseEvent): void {
    ...
    this.camPitch = Math.min(
      MAX_PITCH,
      Math.max(MIN_PITCH, this.camPitch + my * this.lookSensitivity * this.lookPitchSign),
    );
    this.camTargetPitch = this.camPitch;
  ...
  readMoveInput(): MoveInput {
    ...
    // Analog speed: use the max of touch/gamepad multipliers; keyboard has none
    // (undefined = full speed). Clamp to 0–1 for safety.
    const analogMult = Math.min(1, Math.max(
      this.touchMove.moveSpeedMult ?? 0,
      this.gamepadMove.moveSpeedMult ?? 0,
    )) || undefined;

    // RO/L2-style: A/D always turn (no strafe-on-mouselook).
