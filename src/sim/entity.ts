...
export function createNpc(id: number, def: NpcDef, pos: Vec3, rng: { range: (min: number, max: number) => number }): Entity {
  const e = baseEntity(id, pos);
  e.kind = 'npc';
  e.templateId = def.id;
  e.name = def.name;
  e.level = 10;
  e.hostile = false;
  // Combat stats: use def values if provided, otherwise default to passive NPC
  const hpBase = def.hpBase ?? 500;
  const hpPerLevel = def.hpPerLevel ?? 0;
  e.maxHp = Math.round(hpBase + hpPerLevel * (e.level - 1));
  e.hp = e.maxHp;
  // Set weapon if NPC has damage stats
  if (def.dmgBase !== undefined) {
    const dmg = def.dmgBase + (def.dmgPerLevel ?? 0) * (e.level - 1);
    e.weapon = {
      min: Math.round(dmg * 0.8),
      max: Math.round(dmg * 1.25),
      speed: def.attackSpeed ?? 2.0,
    };
  }
  // Armor scales from level like mobs
  if (def.armorPerLevel) {
    e.stats.armor = Math.round(def.armorPerLevel * (e.level - 1));
  }
  e.facing = def.facing;
  ...
  // Randomize initial wander timer so NPCs don't all start moving at once
  e.wanderTimer = rng.range(2, 10);
  return e;
}
