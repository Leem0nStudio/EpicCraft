...
interface CreatureMaterialConfig {
  ...
  roughness: number;
  metalness: number;
  rimIntensity: number; // 0 = no rim, 0.06 = subtle, 0.12 = full
}
...
// Legacy tint record for Lambert fallback
const TINT: Record<Species, number> = {
  rabbit: CREATURE_MATERIALS.rabbit.color,
  squirrel: CREATURE_MATERIALS.squirrel.color,
  bird: CREATURE_MATERIALS.bird.color,
};
...
export function buildCritters(seed: number): CritterField {
  ...
  function matFor(s: Species): THREE.Material {
    ...
    if (GFX.standardMaterials) {
      ...
      // Apply rim-light on high/ultra tiers with per-species intensity
