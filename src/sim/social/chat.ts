...
export function chat(ctx: SimContext, text: string, pid?: number): SentChat | null {
  ...
  // "/jobchange <class>": change from novice to a main class. Only works
  // for novice characters at level 5+.
  const jobChangeMatch = /^\/jobchange\s+(\w+)(?:\s|$)/i.exec(raw);
  if (jobChangeMatch) {
    const targetClass = jobChangeMatch[1].toLowerCase() as PlayerClass;
    const VALID_JOB_CHANGE_CLASSES: PlayerClass[] = [
      'warrior', 'paladin', 'hunter', 'rogue', 'priest', 'shaman', 'mage', 'warlock', 'druid',
    ];
    if (r.meta.cls !== 'novice') {
      ctx.error(r.meta.entityId, 'Only novice characters can change class.');
      return null;
    }
    if (r.e.level < 5) {
      ctx.error(r.meta.entityId, 'You must be at least level 5 to change class.');
      return null;
    }
    if (!VALID_JOB_CHANGE_CLASSES.includes(targetClass)) {
      ctx.error(r.meta.entityId, `Invalid class. Valid classes: ${VALID_JOB_CHANGE_CLASSES.join(', ')}.`);
      return null;
    }
    // Change the class
    ctx.changePlayerClass(r.meta.entityId, targetClass);
    ctx.notice(r.meta.entityId, `You are now a ${targetClass.charAt(0).toUpperCase() + targetClass.slice(1)}!`);
    return null;
  }
