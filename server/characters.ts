...
import type { Gender, PlayerClass } from '../src/sim/types';
...
export interface CharactersRuntime {
  ...
  initialCharacterState(cls: PlayerClass, name: string, skin: number, gender?: Gender): CharacterState;
...
async function createCharacterHandler(ctx: Ctx): Promise<void> {
  ...
  const create = () =>
    charactersDb.createCharacterCapped(
      ...
      cls,
      CHARACTER_LIMIT,
      rt.initialCharacterState(cls, name, skin, gender),
    );
