...
import type { CameraState } from '../game/camera_state';
...
export interface EntityView {
  ...
  /** 2D billboard sprite for character rendering (replaces 3D visual). */
  billboard: import('./billboard').BillboardSprite | null;
}
...
export class Renderer {
  ...
  /** Shared camera state (single source of truth). Set by main.ts. */
  cameraState: CameraState | null = null;
  ...
  private camOcclusion: CameraOcclusionState = { pullT: 1 };
  showNameplates = true;
  ...
  private initialEffectiveRenderScale(scale: number): number {
    // Mobile no longer gets a permanent 85% cap — the adaptive governor handles
    // degradation dynamically, and the initial cap was causing unnecessary
    // softness on capable mobile devices.
    return scale;
  }
  ...
  private createView(e: Entity): void {
    ...
    if (visual) {
    ...
    } else if (this.billboards.has(e.id)) {
      // Billboard sprite: use the mesh as click target
      clickTarget = this.billboards.get(e.id)!.billboard.mesh;
    } else if (body) {
      group.add(body);
      body.traverse((o) => {
      ...
      // Fallback: invisible placeholder so clickTargets has no nulls
      clickTarget = new THREE.Object3D();
      clickTarget.userData.entityId = e.id;
    }
  ...
  private removeView(id: number): void {
    ...
    // Clean up billboard sprite if present
    const billboardEntry = this.billboards.get(id);
    if (billboardEntry) {
      billboardEntry.billboard.dispose();
      // NOTE: Do NOT dispose shared shadow geo/mat — they're shared across all billboards
  ...
  sync(
    ...
    for (const [id, v] of this.views) {
      ...
      // Billboard entities face the camera via their own quaternion (updateFacing),
      // so the entity group must NOT rotate — the parent rotation would compound
      // with the camera quaternion and make the plane face the wrong direction.
      ...
      // Skip visual effects for billboard entities
      if (!this.billboards.has(id)) {
      ...
      // Skip visual-specific updates for billboard entities
      if (!this.billboards.has(id) && active && v.visual) {
      ...
      // Skip visual update for billboard entities
      ...
      if (billboardEntry) {
        ...
        billboard.setAnimation(st.casting ? 'cast' : moving ? 'walk' : 'idle');
        billboard.updateFacing(this.camera);
        billboard.tick(dt);
  ...
  private updateCamera(selfPos: THREE.Vector3, dt: number): void {
    ...
    // Read from shared CameraState (single source of truth).
    ...
    const pz = selfPos.z;
    const eyeY = py + 2.0;
    // Apply combat camera offsets (subtle zoom-in + pitch bump when in combat).
    const effectivePitch = cam.camPitch + cam.combatPitch;
    const effectiveDist = cam.camDist * cam.combatZoom;
    let cx = px - Math.sin(cam.camYaw) * Math.cos(effectivePitch) * effectiveDist;
    let cy = eyeY + Math.sin(effectivePitch) * effectiveDist;
    let cz = pz - Math.cos(cam.camYaw) * Math.cos(effectivePitch) * effectiveDist;
