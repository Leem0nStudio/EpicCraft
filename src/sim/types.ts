...
export interface NpcDef {
  ...
  dynamic?: boolean;
  // Combat stats (optional — NPCs without these are passive)
  hpBase?: number;
  hpPerLevel?: number;
  dmgBase?: number;
  dmgPerLevel?: number;
  attackSpeed?: number; // seconds between swings (default: 2.0)
  armorPerLevel?: number;
  combatProfile?: 'passive' | 'defensive'; // passive = no fight back, defensive = fight when attacked
}
...
export type SimEvent = { pid?: number } & (
  ...
  // Novice class reached level 5 and is ready to change to a main class.
  | { type: 'jobChangeReady' }
...
export interface MoveInput {
  ...
  strafeRight: boolean;
  jump: boolean;
  /** 0–1 analog speed multiplier from touch/gamepad sticks. Undefined = full speed. */
